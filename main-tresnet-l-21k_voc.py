import os, sys, pdb
import argparse
from data import make_data_loader,get_transform,collate_fn,VOC2007
...
parser.add_argument('--save_dir', default='./work/voc2007', type=str, help='save path')
...
def main(args):
    ...
    #只进行验证
    args.resume='heckpoint_best9750.pth'
    args.evaluate=True
    ...
    train_dataset= VOC2007('./data/voc2007', phase='trainval', transform=transform)#训练集位置
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, 
                            num_workers=args.num_workers, pin_memory=True, 
                            collate_fn=collate_fn, drop_last=True)
    transform = get_transform(args, is_train=False)
    val_dataset=VOC2007('./data/voc2007', phase='test', transform=transform)#验证集位置
    ...
    model = tresnet_with_outputlayer(model='tresnet-l', num_classes=num_classes, t=0.4,
                                        adj_file='data/voc/voc_adj.pkl', pretrained=False,
